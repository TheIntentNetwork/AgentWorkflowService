...
class GetSupplementals(BaseTool):
    
    user_id: str = Field(..., description="The user id of the user.")
    
    def run(self, **kwargs):
        try:
            configure_logger("GetSupplementals").info("Running GetSupplementals tool")
            analyzer = AnalyzerEngine()
            analyzer.nlp_engine = "spacy"
            analyzer.language = "en"
            
            # Fetch data from Supabase
            response = Supabase.supabase.table("decrypted_forms").select("decrypted_form").eq("user_id", self.user_id).eq("type", "supplemental").execute()
